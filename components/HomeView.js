...
export default function HomeView() {
  ...
  const navButtons = el('div', { className: 'max-w-4xl mx-auto px-4 py-6 grid grid-cols-2 gap-4' },
    el('button', {
    ...
    el('button', {
      ...
      onClick: () => window.location.hash = '#/exercise-library'
    }, '📚 Exercise Library')
  );
  ...
  if (store.workoutHistory.length > 0) {
    const historySection = el('div', { className: 'max-w-4xl mx-auto px-4 py-6' },
      el('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 p-6' },
        el('h2', { className: 'text-xl font-semibold text-gray-900 mb-4' }, 'Recent Workouts'),
        store.workoutHistory.slice(0, 5).map((workout) =>
          el('div', { className: 'flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0' },
            el('div', {},
              el('p', { className: 'font-medium text-gray-900' }, workout.dayName),
              el('p', { className: 'text-sm text-gray-600' }, `Week ${workout.week} • ${new Date(workout.date).toLocaleDateString()}`)
            )
          )
        )
      )
